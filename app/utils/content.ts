...
export function mapContentNavigation(navigation: any, options?: { labelAttribute?: string }): NavigationTree[] {
  const navMap = {
...
export function findPageHeadline(page): string {
  return page._dir?.title ? page._dir.title : splitByCase(page._dir).map(p => upperFirst(p)).join(' ')
}

export function findPageBreadcrumb(navigation?: any, page?: any): any[] {
  if (!navigation || !page) {
    return []
  }

  return navigation.reduce((breadcrumb: any, link: any) => {
